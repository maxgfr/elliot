...
function addRoom(plusPart, room, id_room_db) {
    ...
    |*                                                                                  *|
    |*  At the beginning, the function tests if the room we want to add exists. In this *|
    |*  case, we do nothing.                                                            *|
    ...
    if (roomID == null) {
        var mainId = document.getElementById('main_home'); // Set referenceto append child later.
...
// Set the table of rooms and their dedicated sensor's parameters.
...
// Set the name of the displayed room icon.
...
// Allow the intensity of the color of a sensor's parameter to vary according to its value.
function darkenColor(color, percentage) {
    ...
    |*  If we want to lighten, the percentage has to be set negative.                   *|
    |*                                                                                  *|
    \************************************************************************************/
...
function removeFromArray(array, element) {
    ...
    if (index !== -1) {
        // Check if the element really exists in the array.
        array.splice(index, 1);
    }
}


// Set the cursor style from a given array.
...
// Prepare and send data in an url.
...
// Set the visual when clicking an element.
function setContrast(element) {
    /*********************************DOCUMENTATION**************************************\
    |*                                                                                  *|
    |*  When clicking on an element, this element becomes darker than at first.         *|
    |*  After that, the element is pushed into the deleted_elements array.              *|
    |*                                                                                  *|
    |*  If the element is dark at first, the element becomes lighter when clicking.     *|
    |*  After that the clicked element is removed from the deleted_elements array.      *|
    |*                                                                                  *|
    \************************************************************************************/
    if (element.className == 'iconPart') {
        ...
        if (element.style.WebkitFilter == 'contrast(100%)') {
            // Check if the icon is unchecked.
            ...
            for (var i = 1; i < element_sensor.length; i++) {
                // Loop through all the sensors and check everything.
            ...
            // Uncheck everything and remove all the elements relative to the actual room in the deleted_elements array.
        ...
        if (element.parentElement.parentElement.previousElementSibling.style.WebkitFilter == "contrast(30%)") {
            /*
               Check if the room is already checked.
...
// Set the visual for additionnal cell preparation.
function addPlusIconForSensors(roomClassName, addToDatabase) {
    ...
    for (var i = 0; i < roomClass.length; i++) {
        ...
        // If we take the color of a sensor, the sensor cannot exist and the color cannot be set.
...
// Set the dropdown list when choosing among several room/parameter icons.
...
// Set the "delete" cross button and its feature.
...
// Set the general model to insert new displayed elements in the database.
...
// Prepare the query string to add a sensor.
...
// Add a given sensor in the databse.
...
// Prepare the query string to add a room.
...
// Add a given room in the database.
...
// Keep a record of the newly created rooms.
...
// Keep a record of the pre-existing rooms.
...
function add_sensor_room(target) {
    ...
    // to show a real distinction between adding and displaying parts.
...
// Return the index of a given value in a given array.
...
// Set a random value in a given sensor's parameter.
...
// Define the basic function to send all the user's modification to the database.
    ...
    for (var i = 0; i < roomPart.length; i++) {
        ...
        for (var j = 0; j < sensorsPart.length; j++) {
            if (sensorsPart[j].includes(roomPart[i])) {
                ...
                // If the sensor is linked to the room (for example, 'bedroom_number_1_toAdd'),
...
// Set the general model to delete new displayed elements in the database.
var _funcDeleteSensorRoom = function() { delete_sensor_room(this) };
var _funcDeleteFromDatabase = function() { delete_from_database(this) };


// Allow for removal of a displayed sensor's parameter.
...
// // Set the general model to cancel the user's modifications.
var _funcCancelModifications = function() { cancel_modifications(true) };


// Withdraw the queries associated to the modification phase.
